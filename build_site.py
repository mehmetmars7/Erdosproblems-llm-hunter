...
Note: Problem statements are NOT included - users are directed to
external sources (erdosproblems.com and MathOverflow).
"""
...
def parse_attack(content, model_name, date_posted=None):
    ...
    # Determine status from FINAL section
    status = 'unknown'
    final_section = sections.get('FINAL', '')
    # Check for UNRESOLVED first since SOLVED is a substring
    if '**UNRESOLVED**' in final_section or final_section.strip().upper().startswith('UNRESOLVED'):
        status = 'unresolved'
    elif '**PARTIAL**' in final_section or 'PARTIAL' in final_section.upper():
        status = 'partial'
    elif '**SOLVED**' in final_section or final_section.strip().upper().startswith('SOLVED'):
        status = 'solved'
